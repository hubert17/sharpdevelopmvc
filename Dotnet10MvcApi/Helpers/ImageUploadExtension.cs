...
namespace Dotnet10MvcApi.Helpers
{
    public static class ImageUploadExtension
    {
        ...
        private const bool QUALITY = true;
        ...
        public static byte[]? ToImageByteArray(this IFormFile file, int maxHeight = MAX_HEIGHT, bool highQuality = QUALITY)
        {
        ...
        public static string SaveAsImageFile(this IFormFile file, string strFileName = "", string strFolder = FOLDER, int maxHeight = MAX_HEIGHT, bool highQuality = QUALITY)
        {
            ...
            try
            {
                ...
                string folder = Path.Combine(Directory.GetCurrentDirectory(), "wwwroot", string.IsNullOrEmpty(strFolder) ? FOLDER : strFolder);
        ...
        public static byte[]? Resize(this byte[] image, string fileFormat, int maxHeight, bool highQuality = QUALITY)
        {
            ...
            using (var img = Image.FromStream(stream))
            {
                ...
                {
                    var prop = processedImg.GetPropertyItem(274);
